...
def random_quat(rand=None):
    ...
    t2 = pi2 * rand[2]
    return np.array(
        (np.cos(t1) * r1, np.sin(t2) * r2, np.cos(t2) * r2, np.sin(t1) * r1))
...
def mat2quat(rmat, precise=False):
    ...
    Returns:
        vec4 float quaternion (w, x, y, z) angles
    """
...
def quat2mat(quaternion):
    """
    Converts given quaternion (w, x, y, z) to matrix.
...
def mat_angVel2quatVel(quat):
    a = -quat[1:4].T
    b = quat[0] * np.eye(3) + cross_mat(quat[1:4])
    ang2quat = 0.5 * np.vstack((a, b))
    return ang2quat


def mat_quatVel2angVel(quat):
    return np.linalg.pinv(mat_angVel2quatVel(quat))
...
def parseQuat(quat):
    assert abs(np.linalg.norm(quat) - 1) < 1e-9
